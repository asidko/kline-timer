...
enum BinanceClient {
    ...
    /// Binance returns each candle as a heterogeneous array; only indices 1–4
    /// (open, high, low, close, as decimal strings) matter to us.
    private static func decode(_ data: Data) throws -> [Candle] {
        ...
        return try rows.map { row in
            guard row.count >= 5,
                  let open = price(row[1]), let high = price(row[2]),
                  let low = price(row[3]), let close = price(row[4]) else {
                throw Failure.malformed
            }
    ...
    /// A kline field is a JSON string like "63100.10"; tolerate a raw number too.
